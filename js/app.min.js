...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (modal && modalBtn && modalClose && modalOverlay) {
    ...
    modalBtn.forEach(function (item, i) {
      item.addEventListener('click', function (event) {
        ...
        document.body.classList.add('scroll-disabled');
        if (document.getElementById(modalID).querySelector('.modal__iframe')) {
          $('.modal__iframe').load('get_data.php', {
            'variant': item.dataset.itemId
          });
        }
      });
    });
  ...
  // map animation dot
  var mapItem = document.querySelectorAll('.c-map__item');
  if (mapItem) {
    setTimeout(function () {
      mapItem.forEach(function (child) {
        return child.classList.remove('is--active');
      });
    }, 5000);
  ...
  var audioPlayerCurrentTime = document.querySelector('.player__value--current');
  var audioPlayerTotalTime = document.querySelector('.player__value--length');
    ...
    podcastsPlayer.forEach(function (item, i) {
      item.addEventListener('click', function (event) {
        ...
        if (item.classList.contains('is--active')) {
          ...
          document.querySelectorAll('.play-btn-audio').forEach(function (child) {
